...
bool Game::combatChangeMana(Creature* attacker, Creature* target, CombatDamage& damage)
{
	...
	if (manaChange > 0) {
		...
		if (realManaChange > 0 && !targetPlayer->isInGhostMode()) {
			...
			std::ostringstream strManaGain;
			strManaGain << realManaChange;
			addAnimatedText(strManaGain.str(), targetPlayer->getPosition(), TEXTCOLOR_BLUE);
		}
	} else {
		const Position& targetPos = target->getPosition();
		if (!target->isAttackable()) {
...
void Game::addAnimatedText(const std::string& message, const Position& pos, TextColor_t color)
{
 ...
 void Game::addAnimatedText(const SpectatorHashSet& list, const std::string& message, const Position& pos, TextColor_t color)
{
